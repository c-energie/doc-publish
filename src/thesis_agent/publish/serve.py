"""Serve the rendered site locally: thesis-agent serve [--port 8123]
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(prog="thesis-agent serve")
    ...
    if not (root / "index.html").exists():
        print(f"{root}/index.html not found - run `thesis-agent site` first", file=sys.stderr)
        return 2
