"""Idempotent document -> Notion wiki sync. Entry point: thesis-agent sync

    thesis-agent sync                       publish to THESIS_NOTION_PARENT
    thesis-agent sync --sandbox <page-id>   publish under a scratch page instead
    thesis-agent sync --dry-run             no network writes; reports only
...
State lives in the *document* repo, under `.thesis-agent/` (see config.state_dir):
...
THESIS_CONTEXT_JSON). The open-questions database ID is read from context.json's
...
def load_context_json() -> dict | None:
    ...
    """
    explicit = os.environ.get("THESIS_CONTEXT_JSON", "").strip()
    if explicit:
        return load_json(Path(explicit), None)
    candidates = [Path("../thesis-context/context.json")]
    try:
        candidates.append(config.thesis_repo().parent / "thesis-context" / "context.json")
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(prog="thesis-agent sync")
    ...
    parent = args.sandbox or os.environ.get("THESIS_NOTION_PARENT")
    if not parent and not args.dry_run:
        print("set THESIS_NOTION_PARENT (or pass --sandbox <page id>)", file=sys.stderr)
        return 2
    if manifest["parent"] and parent and manifest["parent"] != parent:
        if args.sandbox:
            ...
            print(f"manifest is bound to parent {manifest['parent']} but "
                  f"THESIS_NOTION_PARENT is {parent}. Refusing: publishing the wiki "
    ...
    refs_db_url = None
    if thesis_repo and thesis_repo.exists():
        entries = databases.bib_entries(thesis_repo)
        ...
        report.append("THESIS_REPO not set/found - References database skipped, "
