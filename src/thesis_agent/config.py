...
`Path(os.environ.get("THESIS_REPO", ""))` resolved to the current directory when unset
...
__all__ = ["ConfigError", "load_env", "thesis_repo", "state_dir", "build_dir",
           "site_dir", "site_build_dir", "dist_dir", "vendor_dir", "publish_repo",
           "corpus_mode", "source_label", "require", "describe"]

#: Filenames that identify a LaTeX document root, used to sanity-check THESIS_REPO.
_DOC_ROOTS = ("main.tex",)

#: Directory name for publishing state, relative to the document repo.
STATE_DIRNAME = ".thesis-agent"
...
def load_env(start: Path | None = None) -> Path | None:
    ...
    Looked for at `$THESIS_AGENT_ENV` if set, otherwise by walking up from `start`
    ...
    """
    explicit = os.environ.get("THESIS_AGENT_ENV")
    if explicit:
        path = Path(explicit)
        if not path.is_file():
            raise ConfigError(f"THESIS_AGENT_ENV={path} is not a file")
        candidates = [path]
    else:
...
def require(name: str, what: str) -> str:
    ...
    if not value:
        raise ConfigError(f"{name} is not set - it should be {what}")
    return value


def thesis_repo() -> Path:
    """The LaTeX document repo. An input, never written to by ingest."""
    path = Path(require("THESIS_REPO", "the path to the LaTeX document repo")).expanduser()
    if not any((path / root).exists() for root in _DOC_ROOTS):
        raise ConfigError(
            f"THESIS_REPO={path} does not look like a LaTeX document repo "
...
def state_dir(create: bool = False) -> Path:
    ...
    Defaults to `<THESIS_REPO>/.thesis-agent` and is *committed there*, deliberately.
    ...
    Override with THESIS_STATE_DIR when the document repo is one you cannot commit to.
    """
    override = os.environ.get("THESIS_STATE_DIR", "").strip()
    path = Path(override).expanduser() if override else thesis_repo() / STATE_DIRNAME
    if create:
...
def build_dir(create: bool = False) -> Path:
    ...
    override = os.environ.get("THESIS_BUILD_DIR", "").strip()
...
def source_label() -> str:
    ...
    return os.environ.get("THESIS_SOURCE_LABEL", "").strip() or thesis_repo().name
...
def publish_repo() -> Path:
    ...
    path = Path(require(
        "THESIS_PUBLISH_REPO", "the path to the repo that hosts the published site")).expanduser()
    if not path.is_dir():
        raise ConfigError(f"THESIS_PUBLISH_REPO={path} does not exist")
    if not (path / ".git").exists():
        raise ConfigError(
            f"THESIS_PUBLISH_REPO={path} is not a git repo - refusing to write a site "
...
def describe() -> str:
    """Human-readable resolution of every setting, for `thesis-agent config`."""
    lines = [f".env: {_ENV_FILE or '(none found)'}"]
    for label, fn in (("THESIS_REPO", thesis_repo), ("state dir", state_dir),
                      ("build dir", build_dir), ("THESIS_PUBLISH_REPO", publish_repo),
                      ("CORPUS_MODE", corpus_mode)):
        try:
            lines.append(f"{label}: {fn()}")
        except ConfigError as exc:
            lines.append(f"{label}: unset - {exc}")
    for name in ("ANTHROPIC_API_KEY", "NOTION_TOKEN", "THESIS_NOTION_PARENT"):
