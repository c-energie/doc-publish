...
its notebooks), so by default this reads `$DOC_PUBLISH_REPO/figures_html`.
"""
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(prog="doc-publish figures")
    ap.add_argument("html_dir", nargs="?",
                    help="directory of .html exports (default: "
                         "$DOC_PUBLISH_REPO/figures_html)")
    ...
    if not figures_path.exists():
        print(f"{figures_path} missing - run `doc-publish build` first", file=sys.stderr)
        return 2
