"""Build the Quarto site from the corpus: doc-publish site
...
Only the emitter differs.
    ...
    doc-publish site              parse -> plan -> qmd -> render -> report
    doc-publish site --no-render  write the qmd project only (no Quarto needed)
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(prog="doc-publish site")
    ...
    entries: list[dict] = []
    entries = references.bib_entries(document_repo)
    ...
    figures = FigureEmitter(
        figures_json,
        [BUILD / "figures_manifest.json", document_repo / "figures_manifest.json"],
        BUILD, SITE)
