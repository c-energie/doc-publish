...
description: Author or revise .doc-publish/prompt.md — the rules that stop a publishing agent from misrepresenting this document. Use when setting up an agent for a document, when `doc-publish check` reports TODO markers in prompt.md, or when a reader has been given a wrong or overconfident answer about the work.
---

# Writing the agent prompt

`.doc-publish/prompt.md` is the only file in the contract that cannot be generated. It is
