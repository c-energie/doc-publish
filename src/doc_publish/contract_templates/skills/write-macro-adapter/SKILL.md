...
description: Write or extend .doc-publish/macros.py so a publishing agent can expand the LaTeX macros this document defines. Use when `doc-publish check` reports macros with no adapter entry, after adding a \newcommand to the preamble, or when published text contains raw LaTeX like \foo{bar}.
---

# Writing the macro adapter

`.doc-publish/macros.py` teaches the flattener how to expand the macros **this document
...
```bash
doc-publish check          # lists defined macros with no adapter entry
...
Check the real signature in `doc_publish/ingest/adapter.py` before writing: it is the
...
```bash
doc-publish build          # then grep the corpus for a stray backslash
...
- **Re-run `doc-publish check` after editing the preamble** — a macro added to the `.sty`
